...
def base_url():
    ...
    if not url:
        pytest.skip("Set NERD_API_URL to the deployed API URL before running smoke tests.")
    return url.rstrip("/")
...
def id_token():
    ...
    if not token:
        ...
        pytest.skip("Set SMOKE_ID_TOKEN to a valid Firebase ID token before running smoke tests.\n"
