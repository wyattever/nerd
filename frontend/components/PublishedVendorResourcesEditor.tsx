// frontend/components/PublishedVendorResourcesEditor.tsx
"use client";

/**
 * Structured vendor-resources editor for the /editor visual editor. Edits
 * the `vendor_resources` array (PublishedResourceLink[]: { text, url }) of
...
interface PublishedVendorResourcesEditorProps {
...
export function PublishedVendorResourcesEditor({
  record,
  onSave,
  onClose,
}: PublishedVendorResourcesEditorProps) {
  // Lazy initializer: runs exactly once on mount, so generating ids here
  // has no render-time side-effect concerns.
  const [rows, setRows] = useState<DraftRow[]>(() =>
    record.vendor_resources.map((r) => ({ id: makeRowId(), text: r.text, url: r.url }))
  );
  const [issues, setIssues] = useState<string[]>([]);
  const [saveAttempted, setSaveAttempted] = useState(false);

  // record.vendor_resources is a plain prop read (no side effect), so
  ...
  const initialResourcesRef = useRef(record.vendor_resources);
  ...
  useEffect(() => {
    ...
    const handleCancel = (event: Event) => {
      ...
      if (isDirtyRef.current && !window.confirm("Discard unsaved changes to vendor resources?")) {
  ...
  const requestClose = useCallback(() => {
    const dialog = dialogRef.current;
    if (!dialog) return;
    if (isDirtyRef.current && !window.confirm("Discard unsaved changes to vendor resources?")) return;
    dialog.close();
  }, []);
  ...
  return (
    ...
    >
      <h2 id={titleId} className="mb-4 text-lg font-bold text-gray-900">
        Edit: Vendor Resources — {record.product_name}
      </h2>

      <div className="flex max-h-[60vh] flex-col gap-4 overflow-y-auto pr-1">
        {rows.length === 0 ? (
          <p className="text-sm text-gray-500">No vendor resources yet.</p>
        ) : (
          rows.map((row, index) => {
