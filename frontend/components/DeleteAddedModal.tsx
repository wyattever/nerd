// frontend/components/DeleteAddedModal.tsx
"use client";

/**
 * Confirmation modal for permanently deleting an Added record -- see
 * EditorPage's handleDeleteAddedConfirm, which does the actual removal and
 * save. Identical in structure to DeleteCandidateModal.tsx (this file
 * exists as its own component, rather than a shared one parameterized by
 * noun, only because the two tabs' delete flows are wired to separate
 * open/close state in EditorPage -- see isDeleteModalOpen vs.
 * isDeleteAddedModalOpen there).
 *
...
interface DeleteAddedModalProps {
  onConfirm: () => void;
  onClose: () => void;
}

export function DeleteAddedModal({ onConfirm, onClose }: DeleteAddedModalProps) {
