// frontend/components/DeletePublishedModal.tsx
"use client";

/**
 * Confirmation modal for permanently deleting a Published record -- see
 * EditorPage's handleDeletePublishedConfirm, which does the actual removal
 * and save. Identical in structure to DeleteCandidateModal.tsx/
 * DeleteAddedModal.tsx (this file exists as its own component, rather than
 * a shared one parameterized by noun, only because each tab's delete flow
 * is wired to its own open/close state in EditorPage -- see
 * isDeleteModalOpen / isDeleteAddedModalOpen / isDeletePublishedModalOpen
 * there).
 *
...
interface DeletePublishedModalProps {
  onConfirm: () => void;
  onClose: () => void;
}

export function DeletePublishedModal({ onConfirm, onClose }: DeletePublishedModalProps) {
