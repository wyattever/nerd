// frontend/app/editor/(routed)/added/AddedListPanel.tsx
"use client";

/**
 * Added tab's filter/sort/list nav + Messages footer -- see
...
/** Used by AddedEditor to report save/delete/promote/field-editor status
 *  into this panel's persisting Messages footer. */
export function useMessages(): MessagesContextValue {
  const ctx = useContext(MessagesContext);
  if (!ctx) throw new Error("useMessages must be used within AddedListPanel");
  return ctx;
}

interface AddedListPanelProps {
  products: PublishedProductRecord[];
  base: string;
  children: ReactNode;
}

export function AddedListPanel({ products, base, children }: AddedListPanelProps) {
  ...
  const filterId = "added-filter";

  return (
    <div className="flex min-h-full">
      <nav
        aria-label="Added products"
          ...
          <footer className="mt-auto p-6 pt-0">
            <div className="w-full rounded-md border border-gray-300 bg-white">
              ...
              <div className="flex flex-col gap-1 p-4">
                <p className="text-sm text-gray-600">Displaying {products.length} added product records.</p>
