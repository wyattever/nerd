// frontend/app/records/(routed)/added/RecordsAddedListPanel.tsx
"use client";

/**
 * Added records list nav + Messages footer -- see
...
interface RecordsAddedListPanelProps {
  products: PublishedProductRecord[];
  base: string;
  children: ReactNode;
}

export function RecordsAddedListPanel({ products, base, children }: RecordsAddedListPanelProps) {
  ...
  const filterId = "records-added-filter";

  return (
    <div className="flex min-h-full">
      <nav
        aria-label="Added records"
          ...
          <div className="w-full rounded-md border border-gray-300 bg-white">
            ...
            <div className="flex flex-col gap-1 p-4">
              <p role="status" aria-live="polite" className="text-sm text-gray-600">
                Displaying {products.length} added product records.
              </p>
            </div>
          </div>
        </footer>
