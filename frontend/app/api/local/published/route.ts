...
export async function GET(): Promise<Response> {
  ...
  const { data, etag } = await readPublishedRaw();
...
export async function POST(request: Request): Promise<Response> {
  ...
  const { etag: currentEtag } = await readPublishedRaw();
  ...
  await writePublishedAtomic(bytes);

  const { etag: newEtag } = await readPublishedRaw();
