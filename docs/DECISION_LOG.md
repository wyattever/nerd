...
- **Status:** SETTLED. `main` now correctly retains `html_override`/`last_updated_at` as backend fields and their `/render`+conversion-layer support. Only the frontend editing UI (textarea, modal/inline editor, the buttons that invoke it) remains parked on `editable-viewer`. Any future work resuming the editable-viewer UI feature should target re-adding ONLY the frontend components — the backend half never left `main` and needs no changes.
