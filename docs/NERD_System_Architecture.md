...
**NCADEMI EdTech Research for the Directory** *Last Updated: July 07, 2026*
...
* **`link_validator_engine.py`**: A standalone, Playwright-based engine (via Crawlee) that performs high-fidelity, browser-rendered link checking. This engine is decoupled from the primary research orchestrator and is available for administrative, on-demand high-fidelity link checking to optimize cloud costs and worker latency.
...
* **On-Demand Validation**: High-fidelity browser validation (via `link_validator_engine.py`) is invoked manually by administrative users as needed, ensuring these checks do not impact the cost or latency of listing generation.
...
4.  **E2E Tests**: `playwright` + `axe-core` to automate the `Inject -> Edit -> Save -> Delete` UI lifecycle and verify WCAG compliance. Validation checks are explicitly triggered as an isolated step in the test suite to verify the administrative validation endpoint.
...
Every research event and manual modification is logged to **BigQuery** (`edtech-agent-2026.telemetry.feedback_logs`), allowing for precision prompt engineering and usage monitoring. Validation events are logged as administrative events, distinct from automated research logs.

---
*N.E.R.D. System Architecture — Version 2.1 (Stable Production Standard)*