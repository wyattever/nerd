...
def load_dataset(
    ...
    def _to_example(record: dict) -> dspy.Example:
        ...
        ).with_inputs("product_name", "product_url", "known_urls")
...
def url_recall_with_feedback(
    ...
    Mode A — Grounding cache available (Phase 2, recommended):
      Scores against `gold.cached_predicted_urls` (URLs the production
      pipeline actually found via grounding). This tells GEPA whether the
      *formatting* is correct — did the model put found URLs in the right
      sections, in the right format?
    ...
    Mode B — No cache (fallback):
      ...
      This gives GEPA signal on schema adherence when Recall can't be scored.

    The textual feedback in both modes specifically names what was wrong
    so the reflection_lm can generate targeted instruction improvements.
    """
    import re
    ...
    # ------------------------------------------------------------------
    # Mode A: Recall-based scoring (preferred)
    # ------------------------------------------------------------------
    golden_set    = set(gold.golden_urls) if gold.golden_urls else set()
    predicted_set = set(gold.cached_predicted_urls) if gold.cached_predicted_urls else set()

    if golden_set and predicted_set:
        ...
        # Score: how many of the pre-resolved grounding URLs made it into output
        hits   = golden_set & output_urls
        misses = golden_set - output_urls

        recall = len(hits) / len(golden_set) if golden_set else 1.0

        feedback_lines = [
            f"Product: {gold.product_name_display}",
            f"Recall: {recall:.0%} ({len(hits)}/{len(golden_set)} golden URLs in output)",
        ]
        if misses:
            feedback_lines.append(
                f"MISSED URLs (were found by grounding but not in your output — "
        ...
        # Also check structural compliance and add to feedback
...
def evaluate_program(
    ...
    logger.info("%s mean score: %.1%%", label, score * 100)
    return float(score)
...
def run_optimization(
    ...
    #   auto="light"  → ~6 candidate prompts (fast, good for first run)
    #   auto="heavy"  → full exploration (use for production optimization)
    #   num_threads   → cap at 2–4 to respect Vertex grounding quota
    #   reflection_minibatch_size=3 → how many failures the reflection_lm
    #                  sees at once; 3 is a good balance for 20–30 products
    #   track_stats=True → log Pareto frontier progress to terminal
    #   use_merge=False  → don't merge GEPA candidates (cleaner for small sets)
    # ------------------------------------------------------------------
    optimizer = dspy.GEPA(
        metric=url_recall_with_feedback,
